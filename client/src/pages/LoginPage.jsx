...
const LoginPage = () => {
  ...
  return (
    <div className='min-h-screen bg-cover flex items-center
    justify-center gap-8 sm:jsutify-evenly max-sm:flex backdrop-blur-2xl'>
      {/*------left------*/}
     ...
     <button  type='submit' className='py-3 bg-gradient-to-r from-purple-400 to-vio;et-600 text-white rounded-md cursor-pointer'>
      {currState === "Sign up" ?"Create Account": "Login Now"}</button>

      <div className='flex items-center gap-2 text-sm text-gray-500'>
        <input type="checkbox"  />
        <p>Agree to the terms and use  & priavacy policy</p>
      </div>

     <div className='flex-flex-col gap-2'>
      {currState === "Sign up"?(
        <p className='text-sm text-gray-600'> Already have a account? <span
        ...
        className='font-medium text-violet-500 cursor-pointer'>Click
           here</span></p>
      )}
     </div>
